...
(function forkitPink(){
	...
	function initialize() {
		...
		dom.ribbon = document.querySelector( '.pink-forkit' );
		dom.curtain = document.querySelector( '.pink-forkit-curtain' );
		dom.closeButton = document.querySelector( '.pink-forkit-curtain .close-buttonz' );
	...
	function update() {
		...
		if( dragging || state === STATE_DETACHED ) {
			...
			var angle = Math.min( 0, Math.max( 50, Math.atan2( dy, dx ) * 180 / Math.PI ) );
