...
//echo $fullName.''.$phoneNumber.''.$email.''.$companyorganization.''.$contactVal.''.$comments.''.$created_date;
		...
		$email_subject = 'Digitology - Contact Us';
...
$sql = 'INSERT INTO tb_contact_us (`id`, `name`, `phone`, `email`, `contact_type`, `company_organization`, `comments`, `created_date`) VALUES (NULL, "'.$fullName.'", "'.$phoneNumber.'", "'.$email.'","'.$contactVal.'","'.$companyorganization.'","'.$comments.'","'.$created_date.'")';
...
if(!empty($result)){
	...
	echo "<div class='isa_success'>
     <i class='fa fa-check'></i>Thank you for your enquiry. We will contact you shortly.</div>";
        ?>
<script>
    $(".isa_success").fadeOut(5000); 
	$("#showHide_contact_mobile").hide(); 
    $("#fullNameMobile").val("");
	$("#phoneNumberMobile").val("");
	$("#emailMobile").val("");
	$("#companyorganizationMobile").val("");
	$(".contactMobile:checked").val("");
	$("#commentsMobile").val("");
	$("#user_captcha_Mobile").val("");
